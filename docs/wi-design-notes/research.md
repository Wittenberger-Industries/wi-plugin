...
- **Why a ledger row lands the moment the completion notification arrives:** the token figure exists
  only in that notification (wi-directory.md's `tokens.md` section is the canonical statement); a row
...
- **Why the tech-choice web contract lives in the agent (1b/1c):** the mode tag is the dispatch
  ...
  evidence and provenance rules) is `agents/wi-researcher.md`, so the skill states the tag and points
...
- **Why the ADR is committed immediately:** research runs on main before build branches, so an ADR +
  index row committed now rides the branch and the PR (wi-directory.md's project-level rule:
...
- **Where the shared MoA mechanics live:** the dispatch markers, layer-2 semantics ("may change
  position; must say why"), who-writes-what, ledger convention, and the `.wi/models.md` config section
...
- **Why the dossier commit comes first:** the gate decision is made against committed artifacts, and
  ...
  and the PR.
- **Why the summary renders inline:** the `.wi/` files are the appendix, not the message; a gate that
...
- **Why the keep-alive re-print happens only at the interactive gate:** the user is present (they just
  ...
  was recorded, not asked) and the handoff already recorded the line; re-printing would suggest wi can
  arm persistence itself, and arming is the user's act, never wi's.

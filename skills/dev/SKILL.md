...
2. **Open the feature folder, or route the edge case first.** Parse flags: `--auto` sets
   ...
   **new / resume / in-flight-overlap / done-collision / roadmap-row** (tells: an in-flight
   ...
   done feature with this name → done-collision; a matching `.wi/roadmap.md` row → roadmap-row).
   ...
   and seed `progress.md` (template in the research skill's `wi-directory.md`). Every Log line, the
...
5. **Design** (skill `wi:research`): research -> plan -> **design gate** (inline summary;
